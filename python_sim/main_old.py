...
def drawing_ellips(x0=0.0, y0=0.0, a=1.0, b=1.0, theta=0.0):
    ...
    rot_vec = rot @ vec + c
    plt.plot(rot_vec[0], rot_vec[1], c='black', zorder=2, label='Obstacles', linewidth=2)
    plt.axis('equal')
...
def draw_equidistant_from_ellipse_data(ellipse_data, d=1.0, num_points=500):
    ...
    plt.plot(x_eq, y_eq, c='blue', linestyle='-', zorder=1, linewidth=2, label="Equidistant")
    plt.axis('equal')
...
dt = 1                            # Time step
time_end = 1500                    # Simulation time
time = np.arange(0, time_end, dt)  # Total simulation time
d = 10                              # Distance from equididstant to objects
alpha = 0

robot = DubinsCar(dt, 0.1, 0.1, 0.02, d, alpha)
robot.init_pose(0, 50, 0)
...
for t in time:
    ...
    # eps_2[0] = eps_2_x[0] - t * 0.01
    # eps_2[1] = eps_2_y[0] + t * 0.01
    # eps_2[-1] = eps_2_rot[0] - t * 0.001
    ...
    if t == 0:
        ...
        robot.goal = vec_ops.normalize_angle_0_to_2pi(azimuth)
        robot.goal = azimuth
...
def plotting_results():
    ...
    plt.plot(robot.x_path, robot.y_path, label="Dubins Car Path", color='red', linestyle='dashed', zorder=2)
